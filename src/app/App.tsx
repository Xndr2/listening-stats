...
} from "../shared/update/update-check";
...
export function buildProviderChangedState(
  registry: Pick<ProviderRegistry, "getActiveId">,
): {
  activeProviderId: string;
...
function App() {
  ...
  const [settingsInitialTab, setSettingsInitialTab] =
    useState<SettingsTab>("tracking");
  const [tourActive, setTourActive] = useState(() =>
    shouldAutoStartTour(__VERSION__),
  );
  const [activePage, setActivePage] = useState<string>(
    () => getPreferences().activePage,
  );
  const [showShare, setShowShare] = useState(false);
  const [remoteAnnouncement, setRemoteAnnouncement] =
    useState<ParsedRemoteAnnouncement | null>(null);
        ...
        const result = await provider.calculateStatsProgressive(
          ...
          (partial, wave, waveError) => {
            ...
            if (waveError) {
              ...
              setStats((prev) =>
                prev ? { ...prev, ...partial } : { ...EMPTY_STATS, ...partial },
              );
  ...
  const periodTabsPeriods = useMemo(
    () => [...periods, WORLD_TAB_PERIOD],
    [periods],
  );
  ...
  const providerName =
    providerRegistry.getActive()?.getProviderInfo().name ?? "Local";
  const capabilities =
  ...
  const availableSectionIds = new Set(
    getSectionsForProvider(activeCaps).map((s) => s.id),
  );

  const prefs = getPreferences();
  const showStreak =
    capabilities?.hasStreakData || activeProviderId === "statsfm";
  void prefsVersion;
  ...
  const handleRefresh = useCallback(async () => {
    if (providerRegistry.getActiveId() === "statsfm") {
      const { statsfmProvider } =
        await import("../shared/stats/statsfm-provider");
  ...
  const renderSectionById = (id: string): React.ReactNode => {
    switch (id) {
      ...
      case "consistency": {
        if (
          sectionSlots.consistency === "loading" ||
          sectionSlots.consistency === "pending"
        )
          return (
            <ConsistencySection
              loading
  ...
  const renderDashboard = () => {
    const overviewDone =
      sectionSlots.overview === "resolved" || sectionSlots.overview === "error";
    const listsDone =
      sectionSlots.lists === "resolved" || sectionSlots.lists === "error";
    const noSectionErrors = Object.values(sectionErrors).every(
      (e) => e == null,
    );
    if (
      overviewDone &&
    ...
    const loadingSections = (
      Object.entries(sectionSlots) as Array<
        [keyof SectionSlots, SectionSlots[keyof SectionSlots]]
      >
    )
    ...
    return (
      <div className="stats-page-content">
        {hasLoading && (
          <div
            className="loading-status-banner"
            role="status"
            aria-live="polite"
          >
            <span className="loading-status-dot" />
            <span>
              Loading {activeRequestLabel} - waiting on{" "}
              {loadingSections.join(", ")}
            </span>
          </div>
        )}
        ...
        <div className="stats-page-scroll">
          ...
          {activeBanner && (
            <AnnouncementBanner
              ...
              onActionClick={
                activeBanner.actionOpensChangelog
          ...
          <AppFooter
            version={__VERSION__}
            onCheckForUpdates={() => void openUpdatesModal()}
          />
        </div>
      )}
      {showSettings && (
