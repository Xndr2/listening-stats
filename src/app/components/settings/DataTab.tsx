  ...
  const handleFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (parseResult.events.length === 0 && parseResult.errors === 0) {
        Spicetify.showNotification(
          "Import failed: file contains no events",
          true,
        );
        setImportPhase("idle");
        return;
  ...
  const handleExportJson = async () => {
    try {
      ...
      const allTimePeriod =
        activeProvider
          ?.getSupportedPeriods()
          .find((p) => p.id.endsWith("all-time") || p.label === "All Time") ??
        LOCAL_PERIODS[4];
          ...
          >
            <div>
              ...
              <div className="settings-sublabel">
                Accepts .csv or .json from a v1 export
              </div>
            </div>
            <button
              type="button"
      ...
      >
        {!confirmWipe ? (
          ...
          <div style={{ width: "100%" }}>
            ...
            >
              This permanently deletes all play history and cannot be undone.
              Are you sure?
            </p>
