...
export const ERROR_MESSAGES: Record<string, string> = {
  ...
  private:
    "Profile is private. Visit stats.fm settings and set your profile to public.",
...
export function ProvidersTab() {
  ...
  const [config, setConfig] = useState<StatsFmConfig | null>(() =>
    readStatsFmConfig(),
  );
  ...
  const [lfmProvConfig, setLfmProvConfig] =
    useState<LastfmProviderConfig | null>(() => readLastfmConfig());
  ...
  const handleLastfmTest = async () => {
    ...
    if (result.valid) {
      ...
      setLastfmError(
        LASTFM_ERROR_MESSAGES[result.reason] ?? "Connection failed",
      );
  ...
  const handleLastfmProviderConnect = async () => {
    const apiKey =
      lfmApiKeyInput.trim() ||
      localStorage.getItem(LS_KEYS.LASTFM_API_KEY) ||
      "";
  ...
  return (
    <div>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && phase !== "connecting")
                  handleConnect();
              }}
      ...
      {!lfmProvConfig ? (
        <div>
          <div className="settings-sublabel" style={{ marginBottom: "8px" }}>
            Connect your Last.fm account to use it as a stats provider (also
            enables World Charts data).
          </div>
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            <input
              type="text"
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && !lfmConnecting)
                  handleLastfmProviderConnect();
              }}
