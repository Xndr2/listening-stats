...
export function UpdateModal({
  ...
  const [copiedWhich, setCopiedWhich] = useState<null | "bash" | "powershell">(
    null,
  );
  const copiedTimerRef = useRef<number | null>(null);

  const bashCmd = receiveBetaUpdates
    ? INSTALL_COMMAND_BASH_PRERELEASE
    : INSTALL_COMMAND_BASH;
  ...
  const changelogHtml =
    changelogMd !== null
      ? markdownLiteToHtml(changelogMd.slice(0, 120_000))
      : "";
  ...
  return createPortal(
    ...
    >
      <div
        className="settings-modal update-modal"
        role="dialog"
        aria-modal="true"
      >
        ...
        {updateInfo?.updateAvailable && updateInfo.prerelease ? (
          <p className="update-modal-note">
            That release is a GitHub pre-release.
          </p>
        ) : null}
        ...
        <div className="settings-row update-modal-pref-row">
          ...
          {Toggle ? (
            <Toggle
              ...
              onSelected={onReceiveBetaUpdatesChange}
            />
          ) : (
            <input
              type="checkbox"
              ...
              onChange={(e) =>
                onReceiveBetaUpdatesChange(e.currentTarget.checked)
              }
            />
          )}
        </div>
        ...
        {changelogErr ? (
          <p className="update-modal-changelog-error">{changelogErr}</p>
        ) : null}
        <div
