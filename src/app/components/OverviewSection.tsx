...
import { formatNumber, formatHour, formatEstimatedPayout } from "../format";
...
function OverviewHero({
  ...
  const showDelta = priorPeriodTotalDuration != null && priorPeriodTotalDuration > 0;
  const deltaPct = showDelta
    ? Math.round(((totalDuration - priorPeriodTotalDuration!) / priorPeriodTotalDuration!) * 100)
    : null;
...
export default function OverviewSection({ stats, activePeriod, loading = false }: OverviewSectionProps) {
  ...
  const statCardsById: Record<string, StatCardSpec | undefined> = {
    ...
    "new-artists":
      stats.newArtistCount != null
        ? {
            ...
            tooltip: "Artists you played in this period that you didn't play in the previous period",
            sub: "discovered",
          }
        : undefined,
    "peak-hour": {
    ...
    "listening-days": {
      value:
        stats.listeningDays != null && stats.listeningDays > 0 ? formatNumber(stats.listeningDays) : "-",
