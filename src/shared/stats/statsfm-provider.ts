...
function deriveAlbumsFromTracks(tracks: SfmTopTrack[]): TopAlbum[] {
  ...
  return Array.from(albumMap.values())
    ...
    .map((a, i) => ({
      rank: i + 1,
      albumUri:
        a.albumUri || `listening-stats:album:${a.albumName}${a.artistName}`,
...
export class StatsFmProvider implements StatsProvider {
  ...
  async calculateStats(period: Period): Promise<StatsResult> {
    ...
    if (tracksErr && artistsErr && statsErr) {
      ...
      throw new StatsFmError(
        classifyStatsFmError(tracksErr.status, tracksErr.message, resetAt),
      );
    }
    ...
    const listeningDays = perDayData?.days
      ...
      : undefined;
    const streak = perDayData?.days
      ? computeStreakFromDays(perDayData.days)
      : 0;
    ...
    if (priorBoundaries && perDayData?.days) {
      ...
      for (const [ymd, day] of Object.entries(perDayData.days)) {
        ...
        if (
          Number.isFinite(ts) &&
          ts >= priorBoundaries.start &&
          ts < priorBoundaries.end
        ) {
          sum += day.durationMs;
        }
      }
    ...
    const topTracks: TopTrack[] = tracks.map((tt) => {
      ...
      return {
        ...
        albumUri:
          prefixUri(tt.track.albums[0]?.externalIds?.spotify?.[0], "album") ??
          "",
    ...
    const topGenres: TopGenre[] = topGenresFromApiOrArtists(
      apiGenreRows,
      artists,
    );
  ...
  async calculateStatsProgressive(
    period: Period,
    onWave: WaveCallback,
  ): Promise<StatsResult> {
    if (!this.config) {
    ...
    const statsPromise = sfmGet<SfmStreamStats>(
      `/users/${username}/streams/stats`,
      rangeParams,
    );
    const recentPromise = sfmGet<SfmRecentStream[]>(
      `/users/${username}/streams/recent`,
      {
        limit: "12",
      },
    );
    const tracksPromise = sfmGet<SfmTopTrack[]>(
      `/users/${username}/top/tracks`,
      rangeParams,
    );
    const artistsPromise = sfmGet<SfmTopArtist[]>(
      `/users/${username}/top/artists`,
      rangeParams,
    );
    const genresPromise = sfmGet<SfmTopGenre[]>(
      `/users/${username}/top/genres`,
      rangeParams,
    );
    ...
    const perDayPromise = sfmGet<SfmPerDayStats>(
      `/users/${username}/streams/stats/per-day`,
      {
        range: "lifetime",
        timeZone,
      },
    );
    const datesPromise = sfmGet<SfmDateStats>(
      `/users/${username}/streams/stats/dates`,
      {
        range,
        timeZone,
      },
    ...
    const wave2Tasks: Promise<void>[] = [
      tracksPromise.then((res) => {
        ...
        topTracks = tracks.map((tt) => {
          ...
          return {
            ...
            artistUri:
              prefixUri(
                tt.track.artists[0]?.externalIds?.spotify?.[0],
                "artist",
              ) ?? `listening-stats:artist:${tt.track.artists[0]?.name ?? ""}`,
            albumName: tt.track.albums[0]?.name ?? "",
            albumUri:
              prefixUri(
                tt.track.albums[0]?.externalIds?.spotify?.[0],
                "album",
              ) ?? "",
            albumArt: tt.track.albums[0]?.image,
            count: streams,
      ...
      perDayPromise.then((res) => {
        ...
        onWave(
          { streak, listeningDays, dailyPlayCounts, priorPeriodTotalDuration },
          2,
        );
      }),
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "[listening-stats] Discarding invalid stats.fm config:",
        err,
      );
