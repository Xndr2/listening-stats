...
export class LastfmProvider implements StatsProvider {
  ...
  async calculateStats(period: Period): Promise<StatsResult> {
    ...
    const [tracksRes, artistsRes, albumsRes, recentRes] =
    ...
    const tracks: LfmTopTrack[] =
      tracksRes.status === "fulfilled" ? tracksRes.value : [];
    const artists: LfmTopArtist[] =
      artistsRes.status === "fulfilled" ? artistsRes.value : [];
    const albums: LfmTopAlbum[] =
      albumsRes.status === "fulfilled" ? albumsRes.value : [];
    const recentAll: LfmRecentTrack[] =
      recentRes.status === "fulfilled" ? recentRes.value : [];

    const playEvents = recentAll.filter((t) => !Number.isNaN(t.playedAt));
    const totalPlays =
      tracks.reduce((s, t) => s + t.playCount, 0) || playEvents.length;
    ...
    const recentPlays: RecentPlay[] = playEvents
    ...
    try {
      for (let page = 2; page <= STREAK_LOOKBACK_PAGES; page++) {
        const pageTracks = await lastfmGetRecentTracks(
          apiKey,
          username,
          RECENT_PAGE_SIZE,
          page,
      ...
      console.warn(
        "[listening-stats] Last.fm streak pagination stopped early:",
        err,
      );
    }
    ...
    const listeningDays = new Set(
      allStreakTracks.map((t) => toLocalDateKey(t.playedAt)),
    ).size;
