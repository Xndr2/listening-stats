  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    if (fields.length < 7) {
      ...
      if (errorDetails.length < MAX_ERROR_DETAILS) {
        errorDetails.push(
          `Row ${rowNum}: expected 7 fields, got ${fields.length}`,
        );
      }
      continue;
    }

    const [track, artist, album, durationRaw, playedRaw, startedRaw, endedRaw] =
      fields;
    ...
    if (Number.isNaN(durationMs) || Number.isNaN(playedMs)) {
      ...
      if (errorDetails.length < MAX_ERROR_DETAILS) {
        errorDetails.push(
          `Row ${rowNum}: invalid numeric field (duration or played ms)`,
        );
      }
      continue;
    }
    ...
    if (
      ...
      if (errorDetails.length < MAX_ERROR_DETAILS) {
        errorDetails.push(
          `Row ${rowNum}: invalid timestamp (Started At: "${startedRaw}")`,
        );
      }
      continue;
    }
    ...
    if (!Number.isFinite(endedAt) || Number.isNaN(endedAt) || endedAt <= 0) {
      ...
      if (errorDetails.length < MAX_ERROR_DETAILS) {
        errorDetails.push(
          `Row ${rowNum}: invalid timestamp (Ended At: "${endedRaw}")`,
        );
      }
      continue;
    }
...
export async function parseJsonEvents(text: string): Promise<ParseResult> {
  ...
  for (let i = 0; i < parsed.length; i++) {
    ...
    if (
      ...
      if (errorDetails.length < MAX_ERROR_DETAILS) {
        errorDetails.push(
          `Row ${rowNum}: invalid numeric field (timestamp or duration)`,
        );
      }
      continue;
    }
    ...
    if (typeof item.trackUri === "string" && item.trackUri) {
      ...
      const uris = await generateSyntheticUris(
        trackName,
        artistName,
        albumName,
      );
...
export async function importFileEvents(
  ...
  const existing = await db.playEvents
    .where("startedAt")
    .anyOf(startedAts)
    .toArray();
  ...
  const existingKeys = new Set(
    existing.map((e) => `${e.startedAt}:${e.trackName}`),
  );
